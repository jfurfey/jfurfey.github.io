...
function loadData() {
    ...
    d3.csv("data/mbl-scientists-2016.csv", function (csv) {
        ...
        d3.csv("data/mbl-fellows-2016.csv", function (csv) {
            dataSource.fellows = csv;
	    d3.csv("data/mbl-residents-2016.csv", function (csv) {
	        dataSource.resident = csv;
		d3.csv("data/mbl-whitman-2016.csv", function (csv) {
		    dataSource.whitman = csv;
            initialize();
        });
		});
		});
...
function initialize() {
    ...
    viz.data(data)
        ...
        .haloKey(function (d) {
            return d.RES_ID; })                    // The property that determines each PAC
        .nodeKey(function (d) {
            return d.SCI_ID; })                    // The property that determines Candidate
        .nodeGroupKey(function (d) {
            return d.AFFIL; })                        // The property that determines candidate Party affiliation
        .value(function (d) {
            return Number(d.SIZE); })    // The property that determines the weight/size of the link path
        ...
        .on("nodeclick",node_onClick);               // Custom node click
    ...
    viz_title = viz.selection().select("svg").append("text").attr("class", "title")
        ...
        .attr("x", viz.width() / 2).attr("y", 40).attr("text-anchor", "middle").style("font-weight",300).text("MBL Scientists 2016");
...
//Custom code for John
function node_onClick(e,d,i) {
    // Example of opening a window with a google search and one of the data values.
    window.open(d.values[0].WEB);
}
    ...
    links.each(function (d) {
        ...
        halos.each(function (d) {
            if (!haloLabels[d.data.key]) {
                ...
                createPacLabel(d.x, d.y,d.data.values[0].RES_AREA);
            }
        })
    });
...
function arc_onMouseOver(e,d,i) {
    ...
    createDataTip(d.x + rect.left  + rect.width/2, (d.y + rect.top + 100),"Research Area", d.data.values[0].RES_AREA,"Total Researchers: " + total);
}
...
var marinebioColor="#2EFFE2";
var microbiomeColor="#FEF3CC";
var ecologyColor="#6CC097";
var neuroColor="#FF94A3";
var cellbioColor="#FFE12E";
var computationColor="#92B1D0";
var evolutionColor="#C700C3";
var genomicsColor="#FF792E";
var modelsColor="red";
var regenerationColor="#007FC7";
var historyColor="#936";
var undefinedColor="#000000"
var demColor="#0543bc";
var mblColor="#005f61";
var fellowColor="#c16622";
var whitmanColor="#7E0000";
...
var customSkin = {
    ...
    link_stroke: function (d, i) {
        return (d.data.RES_ID == "12") ? undefinedColor : (d.data.RES_ID == "1") ? marinebioColor : (d.data.RES_ID == "2") ? microbiomeColor : (d.data.RES_ID == "3") ? ecologyColor : (d.data.RES_ID == "4") ? neuroColor : (d.data.RES_ID == "5") ? cellbioColor : (d.data.RES_ID == "6") ? computationColor : (d.data.RES_ID == "7") ? evolutionColor : (d.data.RES_ID == "8") ? genomicsColor : (d.data.RES_ID == "9") ? modelsColor : (d.data.RES_ID == "10") ? regenerationColor : (d.data.RES_ID == "11") ? historyColor : (d.nodeGroup=="DEM") ? demColor: otherColor;
    ...
    node_stroke: function (d, i) {
        return (d.nodeGroup == "MBL Resident Scientist") ? mblColor : (d.nodeGroup == "MBL Fellow") ? fellowColor : (d.nodeGroup == "Whitman Scientist") ? whitmanColor : (d.nodeGroup=="DEM") ? demColor: otherColor;
    },
